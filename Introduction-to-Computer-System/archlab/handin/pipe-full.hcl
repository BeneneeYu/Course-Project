...
##
## Descrition
## iaddq help add immediate data to a reister
## constant V is immediate data and a register rB
## It is the destination of the result of the computation(valE). 
## In Fetch stage,we use 1 byte for the icode and ifun. There is a register
## rB so another 1 byte is needed. And we also need 8 bytes for instruction constant valC.
## In this stage,PC increments 10 totally.
## In Decode stage, read register rB to valB.
## In Execute stage, compute the sum of valC and valB and save it into valE.
## As a result of which, conditional code could be set.
## In Write Back stage,valE is wrote to destination rB
## In PC stage,the program counter is updated.
## In this case, I update instr_valid, need_regids, need_valC, srcB, dstE(rB)
## aluA(input is valC),aluB(input B is valB),set_cc
#
## iaddq V, rB
